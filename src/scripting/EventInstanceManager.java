...
public class EventInstanceManager {
    ...
    private List<MapleCharacter> chars = new CopyOnWriteArrayList<MapleCharacter>(); //this is messy
    ...
    public void restartEventTimerMillSecond(int time) {
        try {
            ...
            FileoutputUtil.outputFileError(FileoutputUtil.ScriptEx_Log, ex);
            System.out.println("Event name" + em.getName() + ", Instance name : " + name + ", method Name : restartEventTimerMillSecond:\n");
            ex.printStackTrace();
        }
    }
        ...
        try {
            ...
            for (MapleCharacter chr : getPlayers()) {
                if (name.startsWith("PVP")) {
                    chr.getClient().getSession().writeAndFlush(CField.getPVPClock(Integer.parseInt(getProperty("type")), timesend));
                } else {
    ...
    public void registerGuild(MapleGuild guild, MapleMap map) {
        ...
        for (MapleGuildCharacter pc : guild.getMembers()) {
            registerPlayer(map.getCharacterById(pc.getId()));
        }
    }
    ...
    public void startLucid3Parse(int type) {
        for(MapleCharacter chr : getPlayers()){
            if(chr.getMapId() == 450004550){
                chr.getMap().broadcastMessage(MobPacket.BossLucid.Lucid3rdPhase(type));
                chr.getMap().isLucid3ParseStart = true;
                break;
            }
        }
    }

    public boolean isLucid3Parse(){
        for(MapleCharacter chr : getPlayers()){
            if(chr.getMapId() == 450004550){
                return chr.getMap().isLucid3ParseStart;
            }
        }
        return false;
    }

...
public class NPCScriptManager extends AbstractScriptManager {
    ...
    public final void start(final MapleClient c, final String script) {
        start(c, 0, script);
    }
...
//        final Lock lock = c.getNPCLock();
    ...
    public final void startHairRoom(final MapleClient c, final int npc, final String script, byte result, int slot, byte temp) {
//        final Lock lock = c.getNPCLock();
//        lock.lock();
        try {
            if (!cms.containsKey(c) && c.canClickNPC()) {
                ...
                final NPCConversationManager cm = new NPCConversationManager(c, npc, -1, (byte) -1, iv, script);
                cms.put(c, cm);
                ...
                try {
                    iv.invokeFunction("start", (byte) result, slot, (byte) temp); // Temporary until I've removed all of start
                } catch (NoSuchMethodException nsme) {
                }
            }
        } catch (final Exception e) {
            System.err.println("Error executing NPC script, NPC ID : " + npc + "." + e);
            FileoutputUtil.log(FileoutputUtil.ScriptEx_Log, "Error executing NPC script, NPC ID : " + npc + " / NPC SCRIPT : " + script + " / " + e);
            dispose(c);
        } finally {
//            lock.unlock();
        }
    ...
    public final void start(final MapleClient c, final int npc, final String script, final String method) {
//        final Lock lock = c.getNPCLock();
//        lock.lock();
        try {
            if (!cms.containsKey(c) && c.canClickNPC()) {
                ...
                //System.out.println("NPCID started: " + npc);
                try {
                    iv.invokeFunction(method); // Temporary until I've removed all of start
                } catch (NoSuchMethodException nsme) {
...
//        lock.lock();
        try {
            if (!cms.containsKey(c) && c.canClickNPC()) {
                ...
                if (c.getPlayer().getGMLevel() > 0 && npc > 0) {
                    MapleNPC npcs = MapleLifeFactory.getNPC(npc);
                    c.getPlayer().dropMessage(5, "[ 엔피시 이름 : " + npcs.getName() + " ] [ 엔피시 코드 : " + npcs.getId() + " ]");
                }
                final ScriptEngine scriptengine = (ScriptEngine) iv;
                final NPCConversationManager cm = new NPCConversationManager(c, npc, -1, (byte) -1, iv, script);
                cms.put(c, cm);
            ...
            System.err.println("Error executing NPC script, NPC ID : " + npc + "." + e+ " "+script);
            FileoutputUtil.log(FileoutputUtil.ScriptEx_Log, "Error executing NPC script, NPC ID : " + npc + " / NPC SCRIPT : " + script + " / " + e);
            dispose(c);
        } finally {
//            lock.unlock();
    ...
    public final void startItem(final MapleClient c, final int npc, final String script) {
//        final Lock lock = c.getNPCLock();
        //      lock.lock();
        try {
            if (!cms.containsKey(c) && c.canClickNPC()) {
                ...
                if (iv == null) {
                    iv = getInvocable("item/notcoded.js", c); //safe disposal
                    if (iv == null) {
                        dispose(c);
                        return;
                    }
                ...
                final ScriptEngine scriptengine = (ScriptEngine) iv;
                final NPCConversationManager cm = new NPCConversationManager(c, npc, -1, (byte) -2, iv, script);
                cms.put(c, cm);
                scriptengine.put("cm", cm);
            ...
            FileoutputUtil.log(FileoutputUtil.ScriptEx_Log, "Error executing Item NPC script, NPC ID : " + npc + "." + e);
            dispose(c);
            //      } finally {
//            lock.unlock();
        }
...
//            final Lock lock = c.getNPCLock();
            ...
            try {
                ...
                dispose(c);
                FileoutputUtil.log(FileoutputUtil.ScriptEx_Log, "Error executing NPC script, NPC ID : " + cm.getNpc() + "/" + cm.getScript() + " : " + e);
//            } finally {
                //              lock.unlock();
            }
        }
    ...
    public final void zeroaction(final MapleClient c, final byte mode, final byte type, final int selection1, final int selection2) {
        if (mode != -1) {
            final NPCConversationManager cm = cms.get(c);
            if (cm == null || cm.getLastMsg() > -1) {
                return;
...
//            final Lock lock = c.getNPCLock();
            ...
            try {
                ...
                System.err.println("Error executing NPC script. NPC ID : " + cm.getNpc() + ":" + e+ " "+cm.getScript());
                dispose(c);
...
//        final Lock lock = c.getNPCLock();
        ...
        try {
            if (!cms.containsKey(c) && c.canClickNPC()) {
                ...
                if (iv == null) {
//                    System.out.println("Unhandled quest: " + quest + " | NPCID: " + npc);
                    dispose(c);
                    return;
                }
                final ScriptEngine scriptengine = (ScriptEngine) iv;

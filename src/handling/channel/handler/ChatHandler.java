...
public class ChatHandler {
    ...
    public static final void GeneralChat(final String text, final byte unk, final MapleClient c, final MapleCharacter chr, final LittleEndianAccessor slea, final RecvPacketOpcode recv) {
        if (text.length() > 0 && chr != null && chr.getMap() != null && !CommandProcessor.processCommand(c, text, CommandType.NORMAL)) {
            ...
            int color = chr.getTier() >= 4 ? 20  : chr.getTier() >= 1 ? 5 : 12; //18 , 15 , 저거 1~30까지 하면되는데 가능? 저거 색상코드인데 
            Item item = null;
                ...
                if (chr.isHidden()) {
                    ...
                    if (chr.isIntern() && !chr.isSuperGM() && unk == 0) {
                        if (text.startsWith("~") && chr.isGM()) {
                            World.Broadcast.broadcastMessage(CField.getGameMessage(1, tier + medalname + GM + chr.getName() + "[W: " + String.valueOf(c.getChannel() == 0 ? 1 : c.getChannel()) + "] : " + text.substring(1)));
                        } else if (unk == 0 || text.startsWith("~")) {
                            int sex = c.getChannel() == 0 ? 1 : c.getChannel();
                            World.Broadcast.broadcastMessage(CField.getGameMessage(color, "채널 : <" + sex + "> " + tier + medalname + GM + chr.getName() + " : " + text.substring(1)));
                        } else {
                        ...
                        if (text.startsWith("~") && chr.isGM()) {
                            ...
                            int sex = c.getChannel() == 0 ? 1 : c.getChannel();
                            if (c.getPlayer().getInventory(MapleInventoryType.EQUIPPED).findById(1143197) != null) { //액션
                                World.Broadcast.broadcastMessage(CField.getGameMessage(2, "채널 : <" + sex + "> " + tier + medalname + GM + chr.getName() + " : " + text.substring(1)));                                 
                            } else if (c.getPlayer().getInventory(MapleInventoryType.EQUIPPED).findById(1142397) != null) { //대균
                                World.Broadcast.broadcastMessage(CField.getGameMessage(8, "채널 : <" + sex + "> " + tier + medalname + GM + chr.getName() + " : " + text.substring(1)));                                 
                            } else if (c.getPlayer().getInventory(MapleInventoryType.EQUIPPED).findById(1142409) != null) { //키스
                                World.Broadcast.broadcastMessage(CField.getGameMessage(17, "채널 : <" + sex + "> " + tier + medalname + GM + chr.getName() + " : " + text.substring(1)));
                            } else if (c.getPlayer().getInventory(MapleInventoryType.EQUIPPED).findById(1142648) != null) { //미니
                                World.Broadcast.broadcastMessage(CField.getGameMessage(30, "채널 : <" + sex + "> " + tier + medalname + GM + chr.getName() + " : " + text.substring(1)));                                 
                            } else if (c.getPlayer().getInventory(MapleInventoryType.EQUIPPED).findById(1142647) != null) { //여제
                                World.Broadcast.broadcastMessage(CField.getGameMessage(4, "채널 : <" + sex + "> " + tier + medalname + GM + chr.getName() + " : " + text.substring(1)));                                
                            } else {
                                World.Broadcast.broadcastMessage(CField.getGameMessage(color, "채널 : <" + sex + "> " + tier + medalname + GM + chr.getName() + " : " + text.substring(1)));                               
                            }
                        } else {

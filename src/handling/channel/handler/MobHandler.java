...
//                    chr.dropMessageGM(5, "---------------------------");
                    ...
                    for (Triple<Integer, Integer, Integer> skill : attack.getSkills()) {
                        ...
                        if (msi != null) {
                            if (skill.right > 0) {
                                msi.setMobSkillDelay(c.getPlayer(), monster, skill.right, (short) 0, (actionAndDir & 0x01) != 0);
                            } else if (msi.getSkillAfter() > 0) {
                                msi.setMobSkillDelay(c.getPlayer(), monster, msi.getSkillAfter(), (short) 0, (actionAndDir & 0x01) != 0);
                            } else {
                                if (option > 0) {
...
//                    chr.dropMessageGM(-8, "mobId : " + monster.getId());
                    //                  chr.dropMessageGM(-8, "skillId : " + skillId);
                    //                chr.dropMessageGM(-8, "skillLevel : " + skillLevel);
//                    chr.dropMessageGM(5, "---------------------------");
                    if (msi != null) {
                        ...
                        } else if (msi.onlyOnce() && monster.getLastSkillUsed(skillId, skillLevel) != 0) {
                            if (chr.isGM()) {

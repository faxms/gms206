...
//        }
            if (itemId == 2023287) {
                c.getPlayer().MakeCDitem(2023287, 2, false);
            }
    ...
    public static boolean UseUpgradeScroll(RecvPacketOpcode header, byte slot, byte ws, byte dst, MapleClient c,
            MapleCharacter chr) {
...
//            legendarySpirit = true; 장인의 혼 프메에서 아무도 안쓸테니까 그냥 지워놓겠음.
            if (header == RecvPacketOpcode.USE_EQUIP_SCROLL || header == RecvPacketOpcode.USE_BLACK_REBIRTH_SCROLL) {
                if (ws < 0) {
                    toScroll = (Equip) chr.getInventory(MapleInventoryType.EQUIPPED).getItem(ws);
                } else {
                    toScroll = (Equip) chr.getInventory(MapleInventoryType.EQUIP).getItem(ws);
                }
            } else if (header == RecvPacketOpcode.USE_UPGRADE_SCROLL && ws == 6) {
                toScroll = (Equip) chr.getInventory(MapleInventoryType.DECORATION).getItem(dst);
            } else {
                toScroll = (Equip) chr.getInventory(MapleInventoryType.EQUIP).getItem(dst);
            }

            if (toScroll == null && dst == 6) {
                toScroll = (Equip) chr.getInventory(MapleInventoryType.DECORATION).getItem(ws);
            }
        }
        if (toScroll == null) {
            return false;
        ...
        if (toUse != null && toUse.getQuantity() >= 1 && toUse.getItemId() == itemId && !chr.inPVP()) {
            switch (toUse.getItemId()) {
                case 2431940: {
                    ...
                    long point = c.getPlayer().getKeyValue(123, "pp") + 20;
                    if (c.getPlayer().getKeyValue(123, "pp") >= pirodo) {
                        c.getPlayer().dropMessage(5, "이미 모든 피로도가 충전되있습니다.");
                        c.getSession().writeAndFlush(CWvsContext.enableActions(c.getPlayer()));
                        return;
                    }
                    if (c.getPlayer().getKeyValue(123, "pp") + 20 > pirodo) {
                        point = pirodo;
                    }
        ...
        switch (itemId) {
            ...
            case 5062503: {
                ...
                if (item != null && c.getPlayer().getInventory(MapleInventoryType.USE).getNumFreeSlot() >= 1) {
                    ...
                    if (c.getPlayer().getInventory(MapleInventoryType.USE).getNumFreeSlot() >= 1) {
                        ...
                        if (equip.getPotential6() > 0) {
                            equip.setPotential4(poten);
                            equip.setPotential5(poten);
                            equip.setPotential6(poten);
                        } else {
                            equip.setPotential4(poten);
                            equip.setPotential5(poten);
                        }
                        c.getSession().writeAndFlush(InventoryPacket.scrolledItem(toUse, item, false, true, pos < 0));
                        c.getSession().writeAndFlush(
                                CField.showPotentialReset(c.getPlayer().getId(), true, itemId, equip.getItemId()));
                        c.getPlayer().forceReAddItem(item,
                                pos < 0 ? MapleInventoryType.EQUIPPED : MapleInventoryType.EQUIP);
                        used = true;
            ...
            case 5062005: { //어메이징 미라클 큐브
                ...
                if (item != null && c.getPlayer().getInventory(MapleInventoryType.USE).getNumFreeSlot() >= 1) {
                    ...
                    eq.setPotential2(potential(item.getItemId(), level));
                    eq.setPotential3(eq.getPotential3() != 0 ? poten : 0);
...
//                        c.getSession().writeAndFlush(CField.showPotentialReset(c.getPlayer().getId(), true, itemId, neq.getItemId()));
            ...
            case 5068305: { // 블랙베리
                if (c.getPlayer().getInventory(MapleInventoryType.CASH).getNumFreeSlot() >= 1 && c.getPlayer().getInventory(MapleInventoryType.EQUIP).getNumFreeSlot() >= 1 && c.getPlayer().getInventory(MapleInventoryType.USE).getNumFreeSlot() >= 1) {
                    int itemid[][] = {{4001716, 1}, {4001716, 1}, {4001716, 1}, {2438145, 1}, {4310261, 600}, {4310261, 800}, {4310261, 900}, {4310261, 350}, {4310261, 340}, {2438514, 1}, {4310261, 366}, {4310261, 500}, {2048753, 250}, {4001716, 1}, {2048753, 115}, {2048753, 100}, {4310261, 550}, {2048753, 100}, {2048753, 110}, {2048753, 120}, {4001716, 1}, {2048753, 130}, {2048753, 200}, {4001716, 1}, {4023026, 1}, {4001716, 1}, {4001716, 1}, {2023287, 1}, {4001716, 1}, {4001716, 1}, {4001716, 1}, {4001716, 1}, {4001716, 1}, {4001716, 1}, {4001716, 1}, {4001716, 1}, {4001716, 3}};
            ...
            case 5068300: { // 위습의 원더베리
                if (c.getPlayer().getInventory(MapleInventoryType.CASH).getNumFreeSlot() >= 1 && c.getPlayer().getInventory(MapleInventoryType.EQUIP).getNumFreeSlot() >= 1 && c.getPlayer().getInventory(MapleInventoryType.USE).getNumFreeSlot() >= 1) {
                    int itemid[] = {5069100, 5002226, 5002227, 5002228, 5002229, 5002230, 5002231, 5000939, 5000940, 5689005, 5240169}; //5069100
                    int rand = Randomizer.nextInt(10000);
                    int i;
                    if (rand < 996) {
    ...
    public static final void pickupItem(MapleMapObject ob, MapleClient c, MapleCharacter chr) {
        ...
        try {
            ...
            if (mapitem.getItemId() == 2431174 || mapitem.getItemId() == 2434021) { // 명예의 훈장.
                int rand = Randomizer.rand(5000, 10000);
            ...
            if (mapitem.getMeso() > 0) {
                ...
                if (mapitem.getItem().getItemId() == 4001886) {
                    if (chr.getLastBossId() != 0) {
                        ...
                        mapitem.getItem().setReward(new BossReward(chr.getInventory(MapleInventoryType.ETC).countById(4001886) + 1, chr.getLastBossId(), party, price * price * 20));
                        chr.setLastBossId(0);
                    } else {

...
public class HyperHandler {
    ...
    public static void getHyperSkill(LittleEndianAccessor rh, MapleClient c) {
        ...
        int lvl = rh.readInt();
        int sp = rh.readInt();
        c.getSession().writeAndFlush(CWvsContext.updateHyperSp(value, lvl, sp, value.equals("incHyperStat") ? 3 + (c.getPlayer().getLevel() - 140) / 10 : value.equals("needHyperStatLv") ? table[lvl] : 1));
    }

...
public class PlayerHandler {
    ...
    public static final void UseTitle(final LittleEndianAccessor slea, final MapleClient c, final MapleCharacter chr) {
        ...
        if (itemId <= 0) {
            ...
            chr.updateInfoQuest(7290, String.valueOf(itemId));
        }
 ...
 /*  if (chr.getMapId() == FishingHandler.FishingMap && itemId == FishingHandler.FishingChair && !chr.Fishing()) {
        ...
        if (chr.getMapId() == ServerConstants.WarpMap) {
            ...
            chr.getClient().getSession().writeAndFlush(CField.UIPacket.detailShowInfo("의자에 앉아 있을시 1분마다 네오잼 5개를 자동으로 수급됩니다.", false));
...
//        c.getSession().writeAndFlush(CWvsContext.enableActions(chr));
    ...
    public static final void TakeDamage(final LittleEndianAccessor slea, final MapleClient c, final MapleCharacter chr) {
        // System.out.println(slea+"데미지리시브");
...
//                    SkillFactory.getSkill(51001005).getEffect(chr.getSkillLevel(51001005)).applyTo(chr, false);
        ...
        if (damage > 0) {
            ...
            if (type == -1 || type == 0) {
                if (chr.getBuffedValue(MapleBuffStat.BlessOfDarkness) != null) {
                ...
                } else if (chr.getBuffedValue(MapleBuffStat.MagicGaurd) != null || chr.getSkillLevel(12000024) > 0 || chr.getSkillLevel(27000003) > 0) {
    ...
    public static final void SpecialMove(final LittleEndianAccessor slea, final MapleClient c, final MapleCharacter chr) {
        ...
        if (linkEffect.getCooldown(chr) > 0 && !effect.ignoreCooldown(chr)) {
            if (chr.skillisCooling(linkEffect.getSourceId()) && !GameConstants.isCooltimeKeyDownSkill(skillid) && !GameConstants.isNoApplySkill(skillid) && !chr.getBuffedValue(skillid) && skillid != 155001104 && skillid != 155001204 && chr.unstableMemorize != skillid) {
                c.getSession().writeAndFlush(CWvsContext.enableActions(c.getPlayer()));
                return;
        ...
        switch (skillid) {
            ...
            case 155111207: { // 돌아오는 증오
                ...
                MapleAtom atom = new MapleAtom(false, chr.getId(), 48, true, 155111207, chr.getTruePosition().x, chr.getTruePosition().y);
                List<Integer> monsters = new ArrayList<>();
                ...
                for (int i = 0; i < size; ++i) {
                    ...
                    if (wreck != null) {
                        monsters.add(0);
                        atom.addForceAtom(new ForceAtom(0, Randomizer.rand(0xA, 0x14), Randomizer.rand(0x5, 0xA), Randomizer.rand(0x4, 0x12D), (short) 0, wreck.getTruePosition()));
                        removes.add(wreck);
            ...
            if (GameConstants.isSoulMaster(chr.getJob()) && chr.getSkillLevel(400011048) > 0) {
                if (!chr.skillisCooling(400011048)) {
                    ...
                    if (skillid != 0) {
                        ...
                        chr.getClient().getSession().writeAndFlush(CField.rangeAttack(skillid, attackList, 1, attack.attPos2, chr.isFacingLeft()));
...
//		basedamage = ((effect.getMoneyCon() * 10) / 100) * effect.getProp(); // Not sure
    ...
    public static final void MagicDamage(final LittleEndianAccessor slea, final MapleClient c, final MapleCharacter chr, boolean chilling, boolean orbital) {
        ...
        if (effect.getCooldown(chr) > 0 && !effect.ignoreCooldown(chr)) {
            if (chr.skillisCooling(effect.getSourceId()) && !GameConstants.isCooltimeKeyDownSkill(effect.getSourceId()) && !GameConstants.isNoApplySkill(attack.skill) && !GameConstants.isLinkedSkill(effect.getSourceId()) && !chr.getBuffedValue(effect.getSourceId()) && chr.unstableMemorize != effect.getSourceId() && effect.getSourceId() != 2301002) {
                c.getSession().writeAndFlush(CWvsContext.enableActions(c.getPlayer()));
                return;
            }
            if (effect.getSourceId() == 15111022 || effect.getSourceId() == 15120003) {
    ...
    public static final void MovePlayer(final LittleEndianAccessor slea, final MapleClient c, final MapleCharacter chr) {
        ...
        if (!chr.getBuffedValue(80002419)) {
            if (chr.getKeyValue(999, "DamageTear") > 0 || chr.getKeyValue(999, "ExpTear") > 0 || chr.getKeyValue(999, "DropTear") > 0 || chr.getKeyValue(999, "CridamTear") > 0 || chr.getKeyValue(999, "BossdamTear") > 0) {
...
//            if (!samepos && c.getPlayer().getBuffSource(MapleBuffStat.DARK_AURA) == 32120013) { //?대뱶諛댁뒪???ㅽ겕 ?ㅻ씪
    ...
    public static final void ChangeMap(final LittleEndianAccessor slea, final MapleClient c, final MapleCharacter chr) {
        ...
        if (slea.available() != 0) {
            ...
            final boolean wheel = slea.readShort() > 0 && chr.haveItem(5510000, 1, false, true) && chr.getMapId() / 1000000 != 925;
...
/*        slea.readShort(); // skillLevel
    ...
    public static void RespawnLucid(MapleClient c) {
        MapleCharacter chr = c.getPlayer();
        if (chr.getDeathCount() > 0 || chr.liveCounts() > 0) {
            final MapleMap to = chr.getMap();
            chr.changeMap(to, to.getPortal(0));
...
//            if (victim.getId() != player.getId()) {
            ...
            if (incPMdR > 0) {
                statups.put(MapleBuffStat.IndiePmdR, new Pair<>(incPMdR, 2000));
            }
            if (incBooster > 0) {
...
//				if (chr.getKeyValue(500862, "openBox") == 6) {
    ...
    public static void arkGauge(int readInt, MapleCharacter chr) {
        ...
        if (chr != null && GameConstants.isArk(chr.getJob())) {
            if (chr.getBuffedValue(155101006)) {
                if (!chr.getBuffedValue(400051334)) {
                    chr.SpectorGauge = Math.max(0, chr.SpectorGauge - 20);
                ...
                int plus = Randomizer.rand(10, 30);

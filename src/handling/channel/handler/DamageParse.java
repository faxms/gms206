...
//                                player.getClient().getSession().writeAndFlush(EffectPacket.showBuffeffect(applyto, (101120207, 3)); //이펙트
                    ...
                    if (player.getSkillLevel(5100015) > 0) {
                        ...
                        if (!player.energyCharge && attack.skill != 400051015) { // 에너지 충전 상태이다.
                            player.energy = Math.min(max, player.energy + add);
                            if (player.energy == max) { // 에너지를 다 채웠다면
                                player.energyCharge = true; // 에너지 차지상태로 넘어간다.
                            }
                        } else { // 에너지 차지 상태이다.
                            int forceCon = 0;
                            ...
                            if (player.energy == 0) { // 에너지를 다 썼다면
                                player.energyCharge = false; // 에너지 충전상태로 넘어간다.
                            }
                        }
...
//                                statusz.add(new Triple<MonsterStatus, MonsterStatusEffect, Long>(MonsterStatus.MS_Blind, new MonsterStatusEffect(attack.skill, effect.getSubTime() > 0 ? effect.getSubTime() : effect.getDuration()), (long) -effect.getX()));
                                ...
                                case 164121044: { // 선기 : 분신 둔갑 태을선인
                                    statusz.add(new Triple<MonsterStatus, MonsterStatusEffect, Long>(MonsterStatus.MS_Freeze, new MonsterStatusEffect(attack.skill, Math.min(5000 + attack.targets - 1, 11000)), (long) 1));
                                    break;
                                }
...
//            }
        ...
        if (totDamage > 0 && player.getSkillLevel(4221013) > 0) {
            ...
            if (player.killingpoint < 3) {
                ...
                if (player.getCooldownLimit(400041039) > 0) {
                    player.changeCooldown(400041039, -500 * 3);
                }
            }
        }
...
//        	e.printStackTrace();
    ...
    public static final AttackInfo parseDmgM(final LittleEndianAccessor lea, final MapleCharacter chr, boolean dot) {
        ...
        try {
            ...
            if (GameConstants.sub_8166C0(ret.skill) || GameConstants.is_screen_attack_skill(ret.skill) || ret.skill == 400010030 || ret.skill == 400031065 || ret.skill == 400031061 || ret.skill == 63121006) {
                lea.readInt();
            }
            ...
            if (ret.skill == 11101220 || ret.skill == 11101221 || ret.skill == 11101120 || ret.skill == 11111120
                    ...
                    || ret.skill == 11121203 || ret.skill == 11121201 || ret.skill == 11121101 || ret.skill == 14111022 || ret.skill == 14111023) {
                ret.charge = lea.readInt();
            }
            ...
            if ((ret.isShadowPartner & 0x4) != 0 && dot && ret.skill != 35121052 //이쯤되면 도트어택일떄 인트 받는게 없는거 아닌가..싶다
                    && ret.skill != 400041041 && ret.skill != 21121068
                    && ret.skill != 400051025 && ret.skill != 400051026
                    && ret.skill != 25111206 && ret.skill != 2100010
                    && ret.skill != 400021031 && ret.skill != 400020002 && ret.skill != 24120055
                    && ret.skill != 33121016 && ret.skill != 33111013
                    && ret.skill != 400031039 && ret.skill != 400031040 && ret.skill != 400040008 && ret.skill != 25111012
                    && ret.skill != 25121055 && ret.skill != 400011060 && ret.skill != 400021049 && ret.skill != 400021050
                    && ret.skill != 400021041 && ret.skill != 400031012 && ret.skill != 151121041 && ret.skill != 61121116) {
                lea.readInt();
            }
            ...
            if (ret.skill == 400011048 || ret.skill == 400011049) {
                lea.skip(4);
            }

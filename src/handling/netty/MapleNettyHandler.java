...
//          }
            ...
            case HOLY_POUNTIN:
                ...
                int skilld = slea.readInt();
                Point pos = slea.readPos();
                if (skilld == 2311011) {
                    PlayerHandler.HolyPountin(c.getPlayer(), objectid, skilld, pos);
                } else if (skilld == 400051076) {
                    PlayerHandler.HandleFullMakerUsed(c.getPlayer(), objectid, skilld, pos);
                }
                break;
            case SPECIAL_STAT:
            ...
            case USE_POTENTIAL_SCROLL:
                try {
                    slea.readInt();
                    InventoryHandler.UseUpgradeScroll(header, (byte) slea.readShort(), (byte) 0, (byte) slea.readShort(), c, c.getPlayer());
                } catch (Exception e) {
                    e.printStackTrace();
                }
                break;
            ...
            case USE_UPGRADE_SCROLL:
                try {
                    slea.readInt();
                    InventoryHandler.UseUpgradeScroll(header, (byte) slea.readShort(), (byte) slea.readShort(), (byte) slea.readShort(), c, c.getPlayer());
                } catch (Exception e) {
                    e.printStackTrace();
                }
                break;
            case USE_EQUIP_SCROLL:
                try {
                    slea.readInt();
                    InventoryHandler.UseUpgradeScroll(header, (byte) slea.readShort(), (byte) slea.readShort(), (byte) slea.readByte(), c, c.getPlayer());
                } catch (Exception e) {
                    e.printStackTrace();
                }
                break;

...
public class DumpItems {
    ...
    private final MapleDataProvider item, string = MapleDataProviderFactory.getDataProvider(new File("wz" + "/String.wz")), character;
    ...
    public void dumpItems() throws Exception {
        ...
        if (!hadError) {
            ...
            PreparedStatement ps = con.prepareStatement("INSERT INTO wz_itemdata(itemid, name, msg, `desc`, slotMax, price, wholePrice, stateChange, flags, karma, meso, itemMakeLevel, questId, scrollReqs, consumeItem, totalprob, incSkill, replaceId, replaceMsg, `create`, afterImage, `forceUpgrade`, `chairType`, `nickSkill`) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
    ...
    public void dumpItem(PreparedStatement psa, PreparedStatement psr, PreparedStatement ps, PreparedStatement pse, MapleData iz) throws Exception {
        ...
        if (doneIds.contains(id) || GameConstants.getInventoryTypeOld(id) == MapleInventoryType.UNDEFINED) {
            return;
        }
        doneIds.add(id);
        ...
        if (smEntry == null) {
            if (GameConstants.getInventoryTypeOld(id) == MapleInventoryType.EQUIP) {
                ret = 1;
            } else {
                ret = 100;
            }
        ...
        if (dat != null) {
            for (MapleData child : dat) {
                equipStats.get(-1).put("mob" + MapleDataTool.getIntConvert("id", child, 0), (int) MapleDataTool.getIntConvert("prob", child, 0));
            }
        }
            ...
            for (MapleData info : dat) {
                ...
                for (MapleData data : info) {
                    if (data.getName().length() > 3 && data.getType() != MapleDataType.CANVAS) {
                        equipStats.get(lv).put(data.getName().substring(3), (int) MapleDataTool.getIntConvert(data, 0));
                    }
                }
            }
        }
                ...
                if (stat.equals("canLevel")) {
                    ...
                    if (stat.equals("skill")) {
                        for (int i = 0; i < d.getChildren().size(); i++) { // List of allowed skillIds
                            rett.put("skillid" + i, (int) MapleDataTool.getIntConvert(Integer.toString(i), d, 0));
                        }
                    } else {
                        final int dd = MapleDataTool.getIntConvert(d, 0);
                        if (dd != 0) {
                            rett.put(stat, (int) dd);
                        }
                    }
                }
            }
        ...
        /*
         * 
         dat = iz.getChildByPath("info/customChair");
         if (dat != null) {
         psa.setInt(1, id);
         psa.setString(2, MapleDataTool.getString(dat, "type"));
         psa.setString(3, "chair");
         psa.setString(4, "1");
         }
         */
        pse.setInt(1, id);
                                ...
                                for (MapleData conK : subKey.getChildren()) {
                                    switch (conK.getName()) {
                                        ...
                                        default:
                                            ...
                                            Integer data = Integer.valueOf(conK.getData().toString());
                                            if (data != null) {

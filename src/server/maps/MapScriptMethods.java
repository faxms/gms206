...
import server.quest.party.MapleNettPyramid.MapleNettPyramidMember;
...
public class MapScriptMethods {
    ...
    public static void startScript_FirstUser(MapleClient c, String scriptName) {
        ...
        c.getPlayer().dropMessageGM(-8, scriptName);
        switch (onFirstUserEnter.fromString(scriptName)) {
            case queen_summon0:{
                if(c.getPlayer().getMap().getAllMonstersThreadsafe().size() == 0){
                    c.getPlayer().getMap().broadcastMessage(CField.startMapEffect("잠든 블러디 퀸에게 말을 걸어보자.",5120025,true));
                }
                break;
            }
            case pierre_Summon:
            case pierre_Summon1:{
                c.getPlayer().getMap().broadcastMessage(CField.environmentChange("rootabyss/firework",19));
                c.getPlayer().getMap().broadcastMessage(CField.EffectPacket.showEffect(null,0,0,34,0,0,(byte)0,true,null,"Field.img/rootabyss/firework",null));
                c.getPlayer().getMap().broadcastMessage(CField.startMapEffect("피에르의 티파티에 온 것을 진심으로 환영한다네!",5120098,true));
                break;
            }
            case dojang_Eff: {
            ...
            case moonrabbit_mapEnter: {
                ...
                c.getPlayer().getMap().lucidButterflyCount = 0;
                c.getPlayer().getMap().setLastButterFlyTime(System.currentTimeMillis());
                break;
            }
...
//                    c.getSession().writeAndFlush(MobPacket.getNodeProperties(shammos, c.getPlayer().getMap()));
            ...
            case pyramidWeather: {
                if (c.getPlayer().isLeader()) {
                    ...
                    if (mnp != null) {
                        for (MapleNettPyramidMember pm : mnp.getMembers()) {
                            if (pm.getChr() != null) {
...
//                System.out.println("Unhandled script : " + scriptName + ", type : onFirstUserEnter - MAPID " + c.getPlayer().getMapId());
    ...
    public static void startScript_User(final MapleClient c, String scriptName) {
        ...
        String data = "";
        if (c.getPlayer().isGM()) {
            c.getPlayer().dropMessage(6, scriptName);
        }
        switch (onUserEnter.fromString(scriptName)) {
            case enter_450004200:{
                c.removeClickedNPC();
                NPCScriptManager.getInstance().start(c, 3003250, "Lucid_phase2");
                break;
            }
            ...
            case cannon_tuto_direction1: {
                ...
                c.getSession().writeAndFlush(UIPacket.getDirectionInfo("Effect/Direction4.img/effect/cannonshooter/balloon/0", 5000, 0, 0, 1, 0));
                c.getSession().writeAndFlush(UIPacket.getDirectionInfo("Effect/Direction4.img/effect/cannonshooter/balloon/1", 5000, 0, 0, 1, 0));
                c.getSession().writeAndFlush(UIPacket.getDirectionInfo("Effect/Direction4.img/effect/cannonshooter/balloon/2", 5000, 0, 0, 1, 0));
            ...
            case Fritto_Dancing_Enter: {
                c.getPlayer().setFrittoDancing(new FrittoDancing(2));
                c.getPlayer().getFrittoDancing().start(c);
                break;
            }
            case magnus_enter_HP: {
                if (c.getPlayer().getMap().getNumMonsters() == 0) {
                    c.getPlayer().getMap().spawnMonsterOnGroundBelow(MapleLifeFactory.getMonster(c.getPlayer().getMapId() == 401060300 ? 8880010 : c.getPlayer().getMapId() == 401060200 ? 8880002 : 8880000), c.getPlayer().getTruePosition());
                }
                break;
            }
            case bhb2_scEnterHp:
            ...
            case Akayrum_ExpeditionEnter: {
                if (c.getPlayer().getMap().getNumMonsters() == 0) {
                    c.getPlayer().getMap().spawnMonsterOnGroundBelow(MapleLifeFactory.getMonster(c.getPlayer().getMapId() == 272020210 ? 8860005 : 8860000), new Point(437, -181));
                }
                break;
            }
            ...
            case Resi_tutor20:
                ...
                break;
            case Resi_tutor30:
                c.getSession().writeAndFlush(EffectPacket.showWZEffect("Effect/OnUserEff.img/guideEffect/resistanceTutorial/userTalk"));
                break;
            case Resi_tutor40:
                NPCScriptManager.getInstance().dispose(c);
                c.removeClickedNPC();
            ...
            case merTutorDrecotion10: {
                ...
                c.getSession().writeAndFlush(UIPacket.getDirectionInfo("Effect/Direction5.img/effect/mercedesInIce/merBalloon/6", 2000, 0, -100, 1, 0));
            ...
            case JinHillah_onUserEnter:
                c.getSession().writeAndFlush(CField.enforceMSG("영혼이 타오르는 양초를 힐라가 일정 시간마다 베어 없앨 것이다. 영혼을 빼앗기지 않게 조심하자.", 254, 8000));
                SkillFactory.getSkill(80002543).getEffect(1).applyTo(c.getPlayer(), false);

                c.getSession().writeAndFlush(CField.JinHillah(0, c.getPlayer()));
                c.getSession().writeAndFlush(CField.JinHillah(1, c.getPlayer()));

                if (c.getPlayer().getMap().getReqTouched() > 0) {
                    c.getSession().writeAndFlush(CField.JinHillah(6, c.getPlayer()));
                    c.getSession().writeAndFlush(CField.JinHillah(7, c.getPlayer()));
                }

                if (c.getPlayer().getMap().getSandGlassTime() > 0) {
                    c.getSession().writeAndFlush(CField.JinHillah(4, c.getPlayer()));
                }

                if (c.getPlayer().liveCounts() > 0) {
                    c.getSession().writeAndFlush(CField.JinHillah(3, c.getPlayer()));
            ...
            case mPark_stageEff:
                ...
                break;
            case mPark_Enter: { // 임시처리, 나중에 case별로 추가.
                if (c.getPlayer().getMapId() == 951000000) { // wtf
                    return;
                }
                break;
            }
            ...
            case rienArrow: {
                if (c.getPlayer().getInfoQuest(21019).equals("miss=o;helper=clear")) {
                    ...
                    c.getSession().writeAndFlush(EffectPacket.showWZEffect("Effect/OnUserEff.img/guideEffect/aranTutorial/tutorialArrow3"));
                }
                break;
            }
            case rien: {
...
//                    c.getPlayer().getMap().spawnNpc(3003160, new Point(768, -76));
                }
                break;
            }
            case PTtutor000: {
                //1,x = No fucking clue. Delay? (x doesn't matter at all.)
                //2,x = soft Crashes..no other clue.
                //3,1 = Walk, Left arrow.
                //3,2 = Walk, Right arrow.
                //3,3 = ??
                //3,4 = Walk, Down arrow (hold).
                //3,5 = Walk, Jump + Left arrow.
                //3,6 = Walk, Jump + Right arrow.
                //3,7 = Walk + Jump (No arrow or Up arrow).
                //3,8 = Walk, Down arrow (tap).
                ...
                try {
                    ...
                    c.getSession().writeAndFlush(CField.UIPacket.getDirectionInfo((byte) 3, 1));
                    c.getSession().writeAndFlush(UIPacket.getDirectionInfo("Effect/Direction6.img/effect/tuto/balloonMsg0/10", 0, 0, -110, 1, 0));
                    Thread.sleep(1300);
            ...
            case enter_993029200: { //링크 사냥터
                ...
                if (c.getPlayer().getV("linkMob") != null && count > 0) {
                    int[] smobid = {9010152, 9010153, 9010154, 9010155, 9010156, 9010157, 9010158, 9010159, 9010160, 9010161, 9010162, 9010163, 9010164, 9010165, 9010166, 9010167,
                        9010168, 9010169, 9010170, 9010171, 9010172, 9010173, 9010174, 9010175, 9010176, 9010177, 9010178, 9010179, 9010180, 9010181};
                    int[] smobx = {1736, 1872, 1944, 2074, 2154, 2237, 2368, 2435, 2567, 2647, 2750};
                    int[] smoby = {399, 132, -81};

                    for (int i = 0; i < smobx.length; i++) {
                        for (int g = 0; g < smoby.length; g++) {
                            int id = smobid[Integer.parseInt(c.getPlayer().getV("linkMob"))];
                            MapleMonster mob = MapleLifeFactory.getMonster(id);
                            mob.setOwner(c.getPlayer().getId());
                            c.getPlayer().getMap().spawnMonsterOnGroundBelow(mob, new Point(smobx[i], smoby[g]));
                        }
                    }
                }
            }
            case enter_993001000:
                if (c.getPlayer().getKeyValue(18838, "stage") == -1) {
                ...
                for (int a = 0; a < records.size(); a++) {
                    PlatformerRecord rec = records.get(a);
...
//        c.getSession().writeAndFlush(UIPacket.IntroDisableUI(true));
    ...
    private static void sendDojoClock(MapleClient c) {
        c.getSession().writeAndFlush(CField.getDojoClock(900, (int) ((System.currentTimeMillis() - c.getPlayer().getDojoStartTime() - c.getPlayer().getDojoCoolTime()) / 1000)));
    }
                ...
                for (String s : di.eventQ) {
                    switch (directionInfo.fromString(s)) {
                        case merTutorDrecotion01: //direction info: 1 is probably the time
                            c.getSession().writeAndFlush(UIPacket.getDirectionInfo("Effect/Direction5.img/effect/mercedesInIce/merBalloon/0", 2000, 0, -100, 1, 0));
                            break;
                        case merTutorDrecotion02:
                            c.getSession().writeAndFlush(UIPacket.getDirectionInfo("Effect/Direction5.img/effect/mercedesInIce/merBalloon/1", 2000, 0, -100, 1, 0));
                            break;
                        case merTutorDrecotion03:
                            c.getSession().writeAndFlush(UIPacket.getDirectionInfo(3, 2));
                            c.getSession().writeAndFlush(UIPacket.getDirectionStatus(true));
                            c.getSession().writeAndFlush(UIPacket.getDirectionInfo("Effect/Direction5.img/effect/mercedesInIce/merBalloon/2", 2000, 0, -100, 1, 0));
                            break;
                        case merTutorDrecotion04:
                            c.getSession().writeAndFlush(UIPacket.getDirectionInfo(3, 2));
                            c.getSession().writeAndFlush(UIPacket.getDirectionStatus(true));
                            c.getSession().writeAndFlush(UIPacket.getDirectionInfo("Effect/Direction5.img/effect/mercedesInIce/merBalloon/3", 2000, 0, -100, 1, 0));
                            break;
                        case merTutorDrecotion05:
                            c.getSession().writeAndFlush(UIPacket.getDirectionInfo(3, 2));
                            c.getSession().writeAndFlush(UIPacket.getDirectionStatus(true));
                            c.getSession().writeAndFlush(UIPacket.getDirectionInfo("Effect/Direction5.img/effect/mercedesInIce/merBalloon/4", 2000, 0, -100, 1, 0));
                            EventTimer.getInstance().schedule(new Runnable() {
                        ...
                        case merTutorDrecotion12:
                            c.getSession().writeAndFlush(UIPacket.getDirectionInfo(3, 2));
                            c.getSession().writeAndFlush(UIPacket.getDirectionStatus(true));
                            c.getSession().writeAndFlush(UIPacket.getDirectionInfo("Effect/Direction5.img/effect/mercedesInIce/merBalloon/8", 2000, 0, -100, 1, 0));
                            c.getSession().writeAndFlush(UIPacket.IntroEnableUI(0));
                            break;

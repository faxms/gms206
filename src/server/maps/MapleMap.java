...
public final class MapleMap {
    ...
    private short decHP = 0, createMobInterval = 4000, top = 0, bottom = 0, left = 0, right = 0;
    ...
    private void dropFromMonster(final MapleCharacter chr, final MapleMonster mob, final boolean instanced) {
        ...
        if (chr.getKeyValue(20210113, "orgelonoff") == 1 && !chr.orgelTime) {
            customs.add(new MonsterDropEntry(2632800, 30000, 1, 1, 0));
        }
        ...
        /*if (mob.getStats().isBoss()) {
            ...
            dropBuff = Math.min(dropBuff, 180.0);
        } */
            ...
            for (final MonsterDropEntry de : finals) {
                try {
                    ...
                    if (GameConstants.getInventoryType(de.itemId) == MapleInventoryType.EQUIP && mob.isExtreme()) {
                        droprate = (de.chance < 250 ? de.chance : 250 * (dropBuff / 100.0) * (showdown / 100.0)); // fixed
                    } else {
                        droprate = (de.chance * (dropBuff / 100.0) * (showdown / 100.0));
                    }
        ...
        if (de.itemId == 0) { // meso
            ...
            if (mesos > 0) {
                if (GameConstants.isLinkMap(chr.getMapId())) {
                    ...
                    spawnMobMesoDrop((int) (mesos * (chr.getStat().mesoBuff >= 3000.0 ? 3000.0 / 100.0 : chr.getStat().mesoBuff / 100.0) * chr.getDropMod() * cmServerrate), calcDropPos(pos, mob.getTruePosition()), mob, chr, false, droptype);
                }
                mesoDropped = true;
            }
    ...
    public final void killMonster(final MapleMonster monster, final MapleCharacter chr, boolean withDrops, final boolean second, byte animation, final int lastSkill) {
        ...
        try {
           ...
           /* Date data = new Date();
            ...
            if (monster.getId() == 8800102) {
                int getPoint = Randomizer.rand(1000, 5000); //카쿰 10~100사이로 줌
                chr.gainBossPoint(getPoint); //  범위
                chr.getClient().getSession().writeAndFlush(CField.getGameMessage(8, getPoint + " 보스포인트가 지급되었습니다."));
            }
            if (monster.getId() == 8880200) {
                int getPoint = Randomizer.rand(1000, 5000); //카웅
                chr.gainBossPoint(getPoint); //  범위
                chr.getClient().getSession().writeAndFlush(CField.getGameMessage(8, getPoint + " 보스포인트가 지급되었습니다."));
            }
            if (monster.getId() == 8810018) {
                int getPoint = Randomizer.rand(1, 30); // 노말혼테일
                chr.gainBossPoint(getPoint); //  범위
                chr.getClient().getSession().writeAndFlush(CField.getGameMessage(8, getPoint + " 보스포인트가 지급되었습니다."));
            }

            if (monster.getId() == 8810122) {
                int getPoint = Randomizer.rand(1000, 5000); // 카혼텔
                chr.gainBossPoint(getPoint); //  범위
                chr.getClient().getSession().writeAndFlush(CField.getGameMessage(8, getPoint + " 보스포인트가 지급되었습니다."));
            }

            if (monster.getId() == 8870000) {
                int getPoint = Randomizer.rand(1, 30); // 노말 힐라
                chr.gainBossPoint(getPoint); //  범위
                chr.getClient().getSession().writeAndFlush(CField.getGameMessage(8, getPoint + " 보스포인트가 지급되었습니다."));
            }

            if (monster.getId() == 8870100) {
                int getPoint = Randomizer.rand(1000, 5000); //하드 힐라
                chr.gainBossPoint(getPoint); //  범위
                chr.getClient().getSession().writeAndFlush(CField.getGameMessage(8, getPoint + " 보스포인트가 지급되었습니다."));
            }

            if (monster.getId() == 8840000) {
                int getPoint = Randomizer.rand(10, 200); // 노말 반레온
                chr.gainBossPoint(getPoint); //  범위
                chr.getClient().getSession().writeAndFlush(CField.getGameMessage(8, getPoint + " 보스포인트가 지급되었습니다."));
            }
            if (monster.getId() == 8840014) {
                int getPoint = Randomizer.rand(1000, 5000); // 하드 반레온
                chr.gainBossPoint(getPoint); //  범위
                chr.getClient().getSession().writeAndFlush(CField.getGameMessage(8, getPoint + " 보스포인트가 지급되었습니다."));
            }
            if (monster.getId() == 8860000) {
                int getPoint = Randomizer.rand(1000, 5000); // 노말 아카이럼
                chr.gainBossPoint(getPoint); //  범위
                chr.getClient().getSession().writeAndFlush(CField.getGameMessage(8, getPoint + " 보스포인트가 지급되었습니다."));
            }
            if (monster.getId() == 8820001) {
                int getPoint = Randomizer.rand(1000, 5000); // 노말핑크빈
                chr.gainBossPoint(getPoint); //  범위
                chr.getClient().getSession().writeAndFlush(CField.getGameMessage(8, getPoint + " 보스포인트가 지급되었습니다."));
            }
            if (monster.getId() == 8850011) {
                int getPoint = Randomizer.rand(1000, 5000); // 노말 시그
                chr.gainBossPoint(getPoint); //  범위
                chr.getClient().getSession().writeAndFlush(CField.getGameMessage(8, getPoint + " 보스포인트가 지급되었습니다."));
            }
            if (monster.getId() == 8880002) {
                int getPoint = Randomizer.rand(2, 40); // 노말 매그
                chr.gainBossPoint(getPoint); //  범위
                chr.getClient().getSession().writeAndFlush(CField.getGameMessage(8, getPoint + " 보스포인트가 지급되었습니다."));
            }
            if (monster.getId() == 8880000) {
                int getPoint = Randomizer.rand(1000, 5000); // 하드 매드
                chr.gainBossPoint(getPoint); //  범위
                chr.getClient().getSession().writeAndFlush(CField.getGameMessage(8, getPoint + " 보스포인트가 지급되었습니다."));
            }
            if (monster.getId() == 8920102) {
                int getPoint = Randomizer.rand(1, 20); //노말 블퀸
                chr.gainBossPoint(getPoint); //  범위
                chr.getClient().getSession().writeAndFlush(CField.getGameMessage(8, getPoint + " 보스포인트가 지급되었습니다."));
            }
            if (monster.getId() == 8920000) {
                int getPoint = Randomizer.rand(1000, 5000); //카오스 블퀸
                chr.gainBossPoint(getPoint); //  범위
                chr.getClient().getSession().writeAndFlush(CField.getGameMessage(8, getPoint + " 보스포인트가 지급되었습니다."));
            }
            if (monster.getId() == 8910100) {
                int getPoint = Randomizer.rand(3, 40); // 노발 반반
                chr.gainBossPoint(getPoint); //  범위
                chr.getClient().getSession().writeAndFlush(CField.getGameMessage(8, getPoint + " 보스포인트가 지급되었습니다."));
            }
            if (monster.getId() == 8910000) {
                int getPoint = Randomizer.rand(1000, 5000); // 카오스 반반
                chr.gainBossPoint(getPoint); //  범위
                chr.getClient().getSession().writeAndFlush(CField.getGameMessage(8, getPoint + " 보스포인트가 지급되었습니다."));
            }
            if (monster.getId() == 8930100) {
                int getPoint = Randomizer.rand(3, 40); // 노말 벨룸
                chr.gainBossPoint(getPoint); //  범위
                chr.getClient().getSession().writeAndFlush(CField.getGameMessage(8, getPoint + " 보스포인트가 지급되었습니다."));
            }
            if (monster.getId() == 8500012) {
                int getPoint = Randomizer.rand(3, 40); //노말 파풀
                chr.gainBossPoint(getPoint); //  범위
                chr.getClient().getSession().writeAndFlush(CField.getGameMessage(8, getPoint + " 보스포인트가 지급되었습니다."));
            }
            if (monster.getId() == 8500022) {
                int getPoint = Randomizer.rand(1000, 8000); // 카오스 파풀
                chr.gainBossPoint(getPoint); //  범위
                chr.getClient().getSession().writeAndFlush(CField.getGameMessage(8, getPoint + " 보스포인트가 지급되었습니다."));
            }
            ...
            if (monster.getId() == 8930000) {
                ...
                int getPoint = Randomizer.rand(1000, 5000);
                chr.gainBossPoint(getPoint); //  범위
                chr.getClient().getSession().writeAndFlush(CField.getGameMessage(8, getPoint + " 보스포인트가 지급되었습니다."));
            ...
            if (monster.getId() == 8880101) {
                int getPoint = Randomizer.rand(5000, 15000);
                chr.gainBossPoint(getPoint); //  범위
                chr.getClient().getSession().writeAndFlush(CField.getGameMessage(8, getPoint + " 보스포인트가 지급되었습니다."));
            ...
            if (monster.getId() == 9101078) {
                int getPoint = Randomizer.rand(2000, 5000);
                chr.gainBossPoint(getPoint); //  범위
                chr.getClient().getSession().writeAndFlush(CField.getGameMessage(8, getPoint + " 보스포인트가 지급되었습니다."));
            ...
            if (monster.getId() == 8880405) {
                int getPoint = Randomizer.rand(8000, 30000);
                chr.gainBossPoint(getPoint); //  범위
                chr.getClient().getSession().writeAndFlush(CField.getGameMessage(8, getPoint + " 보스포인트가 지급되었습니다."));
            ...
            if (monster.getId() == 9400080) {
                int getPoint = Randomizer.rand(500, 1500);
                chr.gainBossPoint(getPoint); //  범위
                chr.getClient().getSession().writeAndFlush(CField.getGameMessage(8, getPoint + " 보스포인트가 지급되었습니다."));
            ...
            if (monster.getId() == 9460030) {
                int getPoint = Randomizer.rand(500, 1000);
                chr.gainBossPoint(getPoint); //  범위
                chr.getClient().getSession().writeAndFlush(CField.getGameMessage(8, getPoint + " 보스포인트가 지급되었습니다."));
            ...
            if (monster.getId() == 8950002) {
                int getPoint = Randomizer.rand(4000, 12000);
                chr.gainBossPoint(getPoint); //  범위
                chr.getClient().getSession().writeAndFlush(CField.getGameMessage(8, getPoint + " 보스포인트가 지급되었습니다."));
            ...
            if (monster.getId() == 8950102) {
                int getPoint = Randomizer.rand(50, 100);
                chr.gainBossPoint(getPoint); //  범위
                chr.getClient().getSession().writeAndFlush(CField.getGameMessage(8, getPoint + " 보스포인트가 지급되었습니다."));
            ...
            if (monster.getId() == 8880151) {
                int getPoint = Randomizer.rand(250, 500);
                chr.gainBossPoint(getPoint); //  범위
                chr.getClient().getSession().writeAndFlush(CField.getGameMessage(8, getPoint + " 보스포인트가 지급되었습니다."));
            ...
            if (monster.getId() == 8880302) {
                int getPoint = Randomizer.rand(5000, 20000); //윌이 10~1500
                chr.gainBossPoint(getPoint); //  범위
                chr.getClient().getSession().writeAndFlush(CField.getGameMessage(8, getPoint + " 보스포인트가 지급되었습니다."));
            ...
            switch (monster.getId()) {
                case 9101078: { // 붉은 늑대
                    List<MapleCharacter> chrs = chr.getMap().getAllChracater();
                    for (MapleCharacter players : chrs) {
                        players.gainItem(2433979, 1);
                        players.gainItem(4036531, 1);
                    }
                    break;
                }
                case 8880425: { // 진 힐라
                    List<MapleCharacter> chrs = chr.getMap().getAllChracater();
                    for (MapleCharacter players : chrs) {
                        players.gainItem(4033339, 10);
                        players.gainItem(4031466, 10);
                        players.gainItem(4031831, 1000);
                    }
                    break;
                }
                case 8644655: { // 더스크
                ...
                case 8880519: { // 검마 하드
                    List<MapleCharacter> chrs = chr.getMap().getAllChracater();
                    for (MapleCharacter players : chrs) {
                        if (Randomizer.rand(1, 100) <= 1) {
                            players.gainItem(2437115, 1);
                        }
                        if (Randomizer.rand(1, 10) <= 1) {
                            players.gainItem(5068305, 1);
                        }
                        players.gainItem(2023287, 2);
                        players.gainItem(2437122, 2);
                        players.gainItem(4001715, 20);
                    }
                    break;
                }
                case 8645050: { // 검마 노말
                ...
                case 9460026: { // 진격의 거인
                    ...
                    for (MapleCharacter players : chrs) {
                        if (Randomizer.rand(1, 100) <= 1) {
                            players.gainItem(4023026, 1);
                        }
                        if (Randomizer.rand(1, 1000) <= 1) {
                            players.gainItem(4033836, 1);
                        }
                    }
                    break;
                }
            ...
            /*
             if (!FieldLimitType.Event.check(getFieldLimit())) {
             ...
             } else if (monster.getId() >= 8220022 && monster.getId() <= 8220026) { // eliteBoss 엘보
             eliteBossSchedule.cancel(false);
             ...
             @Override
             public void run() {
             broadcastMessage(CField.enforceMSG("하늘에서 갑자기 아이템이 떨어지고 있어요! 닿으면 얻을 수 있을것 같아요!", 145, 4000));
             broadcastMessage(SLFCGPacket.milliTimer(22 * 1000));

             ScheduledFuture<?> sch = MapTimer.getInstance().register(new Runnable() {
             @Override
             public void run() {
             int rand = Randomizer.rand(15, 30);
             for (int i = 0; i < rand; ++i) {
             Item item = new Item(2432393 + Randomizer.nextInt(3), (byte) 0, (short) 1, (byte) 0);
             int x = Randomizer.nextInt(getRight() - getLeft()) - getRight();
             spawnFlyingDrop(chr, new Point(x, getTop()), new Point(x, getBottom()), item);
             }
             }
             }, 2 * 1000);
             ...
             public void run() {
             sch.cancel(false);
             eliteBossAppeared = false;
             killAllMonsters(true);
             broadcastMessage(CField.stopClock());
             removeDrops();
             broadcastMessage(CField.environmentChange("eliteMonster/gameOver", 5));
             ...
             if (this.killCount++ >= this.eliteRequire) {
             if (this.eliteCount < 20 && monster.getStats().getLevel() >= 60 && !monster.getStats().isBoss()) {
             this.eliteCount++;
             this.killCount = 0;
             ...
             //스킬 나중에 캐싱하자..
             //    eliteMonster.getEliteGradeInfo().add(new Pair<>(211, 12));
             ...
             } else if (!eliteBossAppeared && monster.getStats().getLevel() >= 60) {
             this.eliteCount = 0;
             this.killCount = 0;
             this.eliteRequire = Randomizer.rand(3000, 5000);
             this.eliteBossAppeared = false;

             killAllMonsters(true);

             int rand = Randomizer.nextInt(5);

             String text[] = {"위대한 분을 위하여!", "미천한 것들이 날뛰고 있구나, 크-하하하하하하하핫!!!!!!", "사냥감이 나타났군.", "목표 발견. 제거 행동에 들어간다.", "재미 있겠군. 어디 한 번 놀아볼까."};
             ...
             broadcastMessage(CField.startMapEffect(text[rand], 5120125 + rand, true));

             MapleMonster eliteBoss = MapleLifeFactory.getMonster(8220022 + rand);

             eliteBoss.getEliteGradeInfo().add(new Pair<>(211, 12));

             eliteBoss.getStats().setLevel(monster.getStats().getLevel()); // 어차피 소환할때마다 바꾸므로 상관이 없다 ㅇㅇ..

             eliteBoss.setHp((long) (eliteBoss.getHp() * 750 * eliteBoss.bonusHp())); // 750배로 추정됨

             spawnMonsterOnGroundBelow(eliteBoss, monster.getTruePosition());
             }
             }, 5 * 1000);

             broadcastMessage(CField.getClock(30 * 60));

             ScheduledFuture<?> sch = MapTimer.getInstance().schedule(new Runnable() {
             @Override
             public void run() {
             eliteBossAppeared = false;
             killAllMonsters(true);
        ...
        if (monster.getId() >= 8920000 && monster.getId() <= 8920003 && withDrops && getNumMonsters() == 0) {
            spawnMonsterOnGroundBelow(MapleLifeFactory.getMonster(8920006), new Point(34, 135));
        } else if (monster.getId() >= 8900000 && monster.getId() <= 8900002 && withDrops && getNumMonsters() == 0) {
            spawnMonsterOnGroundBelow(MapleLifeFactory.getMonster(8900003), new Point(570, 551));
...
//            FileoutputUtil.log(FileoutputUtil.Zakum_Log, MapDebug_Log());
        ...
        if (rune == null && !chr.getBuffedValue(80002282) && chr.getEventInstance() == null && !isTown() && !FieldLimitType.Event.check(getFieldLimit())) {
            ...
            if (Randomizer.isSuccess2(50)) {
                MapleRune rune = new MapleRune(Randomizer.rand(0, 4), monster.getPosition().x, monster.getPosition().y, this);//Randomizer.rand(map.getFootholds().getX1(), map.getFootholds().getX2()), Randomizer.rand(map.getFootholds().getY2(), map.getFootholds().getY1()), map);
                spawnRune(rune);
            }
        }
...
//	        final int numShouldSpawn = (GameConstants.isForceRespawn(mapid) ? monsterSpawn.size() : maxRegularSpawn) - spawnedMonstersOnMap.get();
    ...
    public long getLastButterFlyTime() { return lastButterflyTime; }

    public void setLastButterFlyTime(long ltime) { this.lastButterflyTime = ltime; }

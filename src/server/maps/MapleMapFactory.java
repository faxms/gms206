    ...
    public final MapleMap getMap(final int mapid, final boolean respawns, final boolean npcs, final boolean reactors) {
        ...
        if (map == null) {
            ...
            try {
                ...
                map = new MapleMap(mapid, channel, asdf, monsterRate);
                ...
                /*                //load rune data
                 ...
                 if (!map.isTown() && map.getAllMonster().size() > 0) {
                 MapleMonster mob = (MapleMonster) monsters.get(Randomizer.rand(0, monsters.size() - 1));
                 MapleRune rune = new MapleRune(Randomizer.rand(0, 8), mob.getPosition().x, mob.getPosition().y, map);//Randomizer.rand(map.getFootholds().getX1(), map.getFootholds().getX2()), Randomizer.rand(map.getFootholds().getY2(), map.getFootholds().getY1()), map);
                 map.spawnRune(rune);
                 }*/
                //load reactor data
                String id;
                ...
                if (npcs) {
                    ...
                    try {
                        ...
                        rs = ps.executeQuery();
                        while (rs.next()) {
                            MapleMonster mob = MapleLifeFactory.getMonster(rs.getInt("lifeid"));
                            ...
                            if (mob != null) {
                                map.addMonsterSpawn(mob, rs.getInt("mobTime"), (byte) -1, null);
                            } else {
                                System.err.println("[오류] 몬스터 데이터를 만드는중 널 포인터 오류가 발생했습니다.");
                            }
                        }
                        rs.close();
                        ps.close();
                        ...
                        System.err.println("[오류] 몬스터를 DB로부터 불러오는데 오류가 발생했습니다.");
                    } finally {
                        try {
                            if (ps != null) {
                                ps.close();
                ...
                if (mapData.getChildByPath("monsterDefense/wave") != null) {
                    Map<Integer, List<Integer>> monster = new LinkedHashMap<Integer, List<Integer>>();
                        ...
                        for (MapleData mid : wave.getChildren()) {
                            if ((Integer.parseInt(wave.getName()) != 20 && (i == 0 || i % 4 == 0)) || (Integer.parseInt(wave.getName()) == 20 && (i == 0 || i % 2 == 0))) {
                                if (monster.get(Integer.parseInt(wave.getName())) == null) {
                                    monster.put(Integer.parseInt(wave.getName()), new ArrayList<Integer>());
                                }
                                monster.get(Integer.parseInt(wave.getName())).add(MapleDataTool.getInt(mid.getChildByPath("mobID"), 0));
//                                System.out.println("WAVE " + Integer.parseInt(wave.getName()) + " MobID " + MapleDataTool.getInt(mid.getChildByPath("mobID"), 0));
                            }
                            i++;
                        }
                    }
        ...
        for (int i = 0; i <= 10; i++) {
            if (mapData.getChildByPath(String.valueOf(i)) != null && mapData.getChildByPath(i + "/obj") != null) {
                for (MapleData node : mapData.getChildByPath(i + "/obj")) {
                    ...
                    } else if (node.getChildByPath("SN_count") != null) {
                        String name = MapleDataTool.getString("name", node, "");
                        final Environment ev = new Environment(name,
                                MapleDataTool.getIntConvert("x", node, 0),
                                MapleDataTool.getIntConvert("y", node, 0));

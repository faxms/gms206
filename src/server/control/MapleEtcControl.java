            ...
            while (chrs.hasNext()) {
                ...
                if(chr.getMapId() == 261020700) {
                    ...
                    if(chr.getKeyValue(123, "pp") <= 0) {
                        chr.warp(100000000);
                        chr.dropMessage(5, "모든 피로도가 소진되어 마을로 돌아갔습니다.");
                    } else {
                        if(chr.getKeyValue(124, "ppp") > 60000) {
                            ...
                            chr.dropMessage(5, "피로도가 줄어들었습니다. 피로도 : " + chr.getKeyValue(123, "pp"));
                        }
                    }
                }
                ...
                if(time - lastCoreTime >= 1800000 ) { //900000
                    ...
                    for (ChannelServer csrv : ChannelServer.getAllInstances()) {
                        for(MapleCharacter chra : csrv.getPlayerStorage().getAllCharacters1()){
                            chra.dropMessage(-8, "[알림] 칸에 접속하신 용사님의 염원을 모아 "+ret+" 네오 코어를 획득하였습니다.");
                ...
                if (time - chr.lastMacroTime >= 15 * 10 * 1000 && Randomizer.nextInt(10000) < 1 && !chr.getMap().isTown() && chr.getChair() == 0 && chr.getEventInstance() == null) {
                    ...
                    NPCScriptManager.getInstance().start(chr.getClient(), 2007, "macro");
                }

...
public class MapleStatEffect implements Serializable {
    ...
    private short hp, hpFX, hcHp, mp, mhpR, mmpR, pad, padR, mad, madR, pdd, mdef, acc, avoid, hands, speed, jump, psdSpeed, psdJump, mdf,
            ...
            indiePmdR, morph, lv2mhp, lv2mmp, bufftimeR, summonTimeR, killRecoveryR, dotHealHPPerSecondR, targetPlus, targetPlus_5th, nocoolProp; // ar = accuracy rate
...
//        applyto.dropMessageGM(6, "skill " + ((sourceid == 12120013 || sourceid == 12120014) && primary));
        ...
        if (summonMovementType != null && (((sourceid == 400021071 || sourceid == 35111008 || sourceid == 35120002) && primary) || (sourceid != 400021071 && sourceid != 35111008 && sourceid != 35120002))) {
...
//
            if (((sourceid == 400040008 || sourceid == 400041008) && !primary) ||  sourceid == 2111003 || ((sourceid == 12121005 || sourceid == 400001017 || sourceid == 100001261 || sourceid == 151121041 || sourceid == 4221006) && primary) || (sourceid != 12121005 && sourceid != 400001017 && sourceid != 100001261 && sourceid != 151121041 && sourceid != 4221006  && sourceid != 400040008 && sourceid != 400041008)) {
    ...
    private final void applyBuffEffect(final MapleCharacter applyfrom, final MapleCharacter applyto,
        ...
        boolean aftercancel = false, bufftimeR = true, isPetBuff = false;
...
//                        localstatups.put(MapleBuffStat.KeyDownTimeIgnore, new Pair<>(1, localDuration));
            ...
            case 2321052: { // 헤븐즈 도어
                ...
                if (applyto.getBuffedEffect(MapleBuffStat.CooldownHeavensDoor) == null) {
                    localstatups.put(MapleBuffStat.HeavensDoor, new Pair<>(1, 0));
                }
                break;
            }
            case 2321055: {
                bufftimeR = false;
                if (applyto.getBuffedEffect(MapleBuffStat.CooldownHeavensDoor) == null) {
                    localstatups.put(MapleBuffStat.CooldownHeavensDoor, new Pair<>(1, 600 * 1000));
                } else {
                    return;
                }
                break;
...
//                    effect = false;
            ...
            case 4221016: { // 암살 막타
                ...
                if (applyto.getSkillLevel(400041025) > 0 && applyto.shadowerDebuffOid != 0) {
                    localstatups.put(MapleBuffStat.ShadowerDebuff, new Pair<>(applyto.shadowerDebuff, 10000));
                }
                break;
            }
...
//            		localstatups.put(MapleBuffStat.IndieCr, new Pair<>((int) indieCr, localDuration));
            ...
            case 400021077: {
                localstatups.put(MapleBuffStat.IndieDamR, new Pair<>(q2 + applyfrom.peaceMaker * w2, localDuration));
                break;
            }
...
//                    effect = false;
            ...
            case 400051011: { // 에너지 버스트
                localstatups.clear();
                if (showEffect) {
                    localstatups.put(MapleBuffStat.EnergyBurst, new Pair<>(1, localDuration));
                } else {
                    localDuration = (z + (applyto.getEnergyBurst() / 25)) * 1000;
            ...
            case 400051024: { // ICBM 준비
                localstatups.put(MapleBuffStat.NotDamaged, new Pair<>(1, 600));
                localstatups.put(MapleBuffStat.IndieNotDamaged, new Pair<>(1, 600));
                break;
            }
...
//            		effect = false;
            ...
            case 80002419: {//티어 버프
                localstatups.clear();
                if (applyfrom.getKeyValue(999, "DamageTear") > 0) {
                    int tear = (int) applyfrom.getKeyValue(999, "DamageTear");
                    int damup = tear == 8 ? 150 : tear == 7 ? 120 : tear == 6 ? 100 : tear == 5 ? 80 : tear == 4 ? 60 : tear == 3 ? 40 : tear == 2 ? 20 : 10;
                    localstatups.put(MapleBuffStat.IndieDamR, new Pair<>(damup, 0));
                }
                if (applyfrom.getKeyValue(999, "ExpTear") > 0) {
                    int tear = (int) applyfrom.getKeyValue(999, "ExpTear");
                    int expup = tear == 8 ? 50 : tear == 7 ? 40 : tear == 6 ? 30 : tear == 5 ? 23 : tear == 4 ? 18 : tear == 3 ? 13 : tear == 2 ? 7 : 3;
                    localstatups.put(MapleBuffStat.IndieExp, new Pair<>(expup, 0));
                }
                if (applyfrom.getKeyValue(999, "DropTear") > 0) {
                    int tear = (int) applyfrom.getKeyValue(999, "DropTear");
                    int dropup = tear == 8 ? 300 : tear == 7 ? 180 : tear == 6 ? 120 : tear == 5 ? 80 : tear == 4 ? 60 : tear == 3 ? 40 : tear == 2 ? 20 : 10;
                    localstatups.put(MapleBuffStat.DropRate, new Pair<>(dropup, 0));
                }
                if (applyfrom.getKeyValue(999, "MesoTear") > 0) {
                    int tear = (int) applyfrom.getKeyValue(999, "MesoTear");
                    int mesoup = tear == 8 ? 120 : tear == 7 ? 100 : tear == 6 ? 90 : tear == 5 ? 80 : tear == 4 ? 60 : tear == 3 ? 40 : tear == 2 ? 20 : 10;
                    localstatups.put(MapleBuffStat.MesoUp, new Pair<>(mesoup, 0));
                }
                if (applyfrom.getKeyValue(999, "CridamTear") > 0) {
                    int tear = (int) applyfrom.getKeyValue(999, "CridamTear");
                    int criup = tear == 8 ? 100 : tear == 7 ? 70 : tear == 6 ? 50 : tear == 5 ? 30 : tear == 4 ? 20 : tear == 3 ? 15 : tear == 2 ? 10 : 5;
                    localstatups.put(MapleBuffStat.IndieCD, new Pair<>(criup, 0));
                }
                if (applyfrom.getKeyValue(999, "BossdamTear") > 0) {
                    int tear = (int) applyfrom.getKeyValue(999, "BossdamTear");
                    int damup = tear == 8 ? 150 : tear == 7 ? 120 : tear == 6 ? 100 : tear == 5 ? 70 : tear == 4 ? 50 : tear == 3 ? 30 : tear == 2 ? 20 : 10;
                    localstatups.put(MapleBuffStat.IndieBDR, new Pair<>(damup, 0));
                }
                break;
            }
            default:
    ...
    public final SummonMovementType getSummonMovementType() {
        ...
        if (GameConstants.isAngelicBlessSkill(SkillFactory.getSkill(sourceid)) || GameConstants.isAngelicBlessBuffEffectItem(sourceid)) {

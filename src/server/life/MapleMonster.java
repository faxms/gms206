...
//        if (ostats != null) {
    ...
    private final void giveExpToCharacter(final MapleCharacter attacker, long exp, final boolean highestDamage, final int numExpSharers, final byte pty, final byte Class_Bonus_EXP_PERCENT, final byte Premium_Bonus_EXP_PERCENT, final int lastskillID) {
        ...
        if (exp > 0) {
            ...
            if (exp > 0) {
                // 레벨별 경험치 기본 50배 가정
                if (attacker.getLevel() >= 0 && attacker.getLevel() < 200) { //0~199
                    exp *= 300;
                } else if (attacker.getLevel() >= 200 && attacker.getLevel() < 210) { //200~209
                    exp *= 300;
                } else if (attacker.getLevel() >= 210 && attacker.getLevel() < 220) { //210~219
                    exp *= 300;
                } else if (attacker.getLevel() >= 220 && attacker.getLevel() < 230) { //220~229
                    exp *= 300;
                } else if (attacker.getLevel() >= 230 && attacker.getLevel() < 240) { //230~239
                    exp *= 300;
                } else if (attacker.getLevel() >= 240 && attacker.getLevel() < 250) { //240~249
                    exp *= 300;
                } else if (attacker.getLevel() >= 250 && attacker.getLevel() < 260) { //250~259
                    exp *= 300;
                } else if (attacker.getLevel() >= 260 && attacker.getLevel() < 270) { //260~269
                    exp *= 300;
                } else if (attacker.getLevel() >= 270 && attacker.getLevel() < 275) { //270~274
                    exp *= 300;
                } else if (attacker.getLevel() >= 275 && attacker.getLevel() < 280) { //275~279
                    exp *= 300;
                } else if (attacker.getLevel() >= 280 && attacker.getLevel() < 285) { //280~284
                    exp *= 300;
                } else if (attacker.getLevel() >= 285 && attacker.getLevel() < 290) { //285~289
                    exp *= 300;
                } else if (attacker.getLevel() >= 290 && attacker.getLevel() < 295) { //290~295
                    exp *= 300;
                } else if (attacker.getLevel() >= 295 && attacker.getLevel() < 300) { //295~299
                    exp *= 300;
                } else if (attacker.getLevel() >= 300 && attacker.getLevel() < 1000) { //300~999
                    exp *= 300;
                } else {
                    exp *= 1;
                }
...
//                baseExp = (int) Math.ceil(totalBaseExp * ((double) attackEntry.getDamage() / getMobMaxHp()));
        ...
        if (!FieldLimitType.Event.check(map.getFieldLimit())) {
            if (getStats().getLevel() >= killer.getLevel() - 30 && getStats().getLevel() <= killer.getLevel() + 30) {
                if (Randomizer.nextInt(10000) < 8 && map.getPoloFrittoPortal() == null) {
                ...
                if (Randomizer.nextInt(10000) < 5 && map.getFireWolfPortal() == null) {
...
//        if (stati.size() > 0 && stati.containsKey(MonsterStatus.DOOM)) {
    ...
    public final void setLastSkillUsed(MobSkill msi, final long now, long cooltime) {
        usedSkills.put(msi, now + cooltime);
    }

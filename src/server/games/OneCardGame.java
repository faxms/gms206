    ...
    public void endGame(OneCardPlayer winner, boolean error) {
        ...
        for (OneCardPlayer bp : this.getPlayers()) {
            if (bp.getPlayer().getId() == winner.getPlayer().getId()) {
              /*  final int winp  = 500;
                if(bp.getPlayer().getKeyValue(501215, "today") < 0){
                    bp.getPlayer().setKeyValue(501215, "today", 0 + "");
                }
                if(bp.getPlayer().getKeyValue(501215, "today") >= 2000){
                    bp.getPlayer().dropMessage(-8, "하루동안 [미니게임 플레이]로 획득 가능한 네오 코어량을 초과하였습니다.");
                }else{
                    int todaycore = (int) bp.getPlayer().getKeyValue(501215, "point") + winp;
                    int coreg = winp + (int) bp.getPlayer().getKeyValue(501215, "point");
                    bp.getPlayer().setKeyValue(501215, "today", todaycore + "");
                    bp.getPlayer().setKeyValue(501215, "point", coreg + "");
                    bp.getPlayer().dropMessage(-8, "게임에서 승리하여 "+winp+"포인트를 획득하였습니다.");
                    bp.getPlayer().updateInfoQuest(501215, "point=" + bp.getPlayer().getKeyValue(501215, "point") + ";sum=0;date=" + GameConstants.getCurrentDate_NoTime() + ";week=0;total=0;lock=0"); //네오 코어
                }*/
             ...
             /*   final int lostp  = 200;
                ...
                if(bp.getPlayer().getKeyValue(501215, "today") >= 2000){
                    ...
                    int todaycore = (int) bp.getPlayer().getKeyValue(501215, "point") + lostp;
                    int coreg = lostp + (int) bp.getPlayer().getKeyValue(501215, "point");
                    bp.getPlayer().setKeyValue(501215, "today", todaycore + "");
                    bp.getPlayer().setKeyValue(501215, "point", coreg + "");
                    bp.getPlayer().dropMessage(-8, "게임에서 패배하여 "+lostp+"포인트를 획득하였습니다.");
                    bp.getPlayer().updateInfoQuest(501215, "point=" + bp.getPlayer().getKeyValue(501215, "point") + ";sum=0;date=" + GameConstants.getCurrentDate_NoTime() + ";week=0;total=0;lock=0"); //네오 코어
                }*/
                sendPacketToPlayers(SLFCGPacket.OneCardGamePacket.onShowScreenEffect("/Effect/screeneff/gameover"));
            }
